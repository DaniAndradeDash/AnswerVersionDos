...
const accentColors = [
  { border: 'border-secondary', borderHover: 'group-hover:border-secondary/50', bg: 'bg-secondary', text: 'text-secondary', glow: 'shadow-secondary/20', from: 'from-secondary', gradient: 'from-secondary/20' },
  { border: 'border-primary', borderHover: 'group-hover:border-primary/50', bg: 'bg-primary', text: 'text-primary', glow: 'shadow-primary/20', from: 'from-primary', gradient: 'from-primary/20' },
  { border: 'border-secondary', borderHover: 'group-hover:border-secondary/50', bg: 'bg-secondary', text: 'text-secondary', glow: 'shadow-secondary/20', from: 'from-secondary', gradient: 'from-secondary/20' },
]
      ...
      >
        ...
        <div className="relative z-10 flex flex-col h-full p-8">
          ...
          <div className="mt-6 pt-4 border-t border-border/30 flex items-center justify-between">
            <span className={`text-xs font-medium ${accent.text} uppercase tracking-wider`}>
              {isSelected ? 'Cerrar detalles' : 'Ver detalles'}
            </span>
            <div className={`flex h-8 w-8 items-center justify-center rounded-full ${isSelected ? `${accent.bg} text-white` : 'bg-muted/50 text-muted-foreground'} transition-colors duration-300`}>
              {isSelected ? (
          ...
          >
            <div className="pt-5 mt-2 border-t border-border/30 space-y-4">
              ...
              {/* Service image */}
              {service.image && (
                <div className="relative w-full h-40 rounded-xl overflow-hidden border border-border/30">
                  <Image
                    src={service.image}
                    alt={service.title}
                    fill
...
export default function Servicios() {
  ...
  return (
    <Section id="servicios" variant="default" headingId="servicios-heading" className="relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start relative">
          ...
          <div className="lg:col-span-7">
            <AnimatedSection animation="fade-right" duration={0.6}>
              <h2 id="servicios-heading" className="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-extrabold text-foreground tracking-tight leading-[1.1]">
                Nuestros{' '}
          ...
          <div className="lg:col-span-5 lg:pt-4">
            <AnimatedSection animation="fade-left" delay={0.15} duration={0.6}>
              <p className="text-muted-foreground text-base sm:text-lg leading-relaxed">
                En Answer{' '}
                <span className="text-secondary font-semibold">st</span>{' '}
      ...
      >
        {services.map((service, index) => (
          <ServiceCard
            ...
            onToggle={() => setSelected(selected === service.id ? null : service.id)}
          />
        ))}
      </StaggerContainer>
      ...
      <div ref={ctaRef} className="mt-20 relative z-10">
        <div className="relative rounded-2xl border border-border/50 bg-card/80 backdrop-blur-sm overflow-hidden">
          ...
          <div
            className="absolute inset-0 bg-gradient-to-r from-secondary/5 via-primary/5 to-secondary/5"
            aria-hidden="true"
          />

          {/* Left accent line */}
          <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-secondary via-primary to-secondary" aria-hidden="true" />

          <div className="relative z-10 flex flex-col lg:flex-row items-center gap-6 lg:gap-8 p-8 lg:p-10">
            {/* Text content */}
